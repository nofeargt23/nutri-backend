...
const now = () => Date.now();
const cacheGet = (k: string) => { const h = CACHE.get(k); if (!h) return null; if ((now()-h.ts)/1000 > TTL) { CACHE.delete(k); return null; } return h.data; };
const cacheSet = (k: string, v: any) => CACHE.set(k, { ts: now(), data: v });
const fastHash = (b: Buffer) => { let h = 2166136261>>>0; for (let i=0;i<b.length;i++){ h^=b[i]; h=Math.imul(h,16777619);} return (h>>>0).toString(16); };
...
async function callWithTokens(url: string, init: RequestInit = {}) {
  ...
  for (let i=0;i<tokens.length;i++){
    ...
    try {
      ...
      if ([401,403,429].includes(r.status)) { last = new Error(`${r.status}: ${txt.slice(0,200)}`); continue; }
      throw new Error(`${r.status}: ${txt.slice(0,200)}`);
    } catch(e){ last = e; }
  }
  throw last || new Error("All tokens failed");
}
async function postJSON(path: string, body: any){ const r = await callWithTokens(`${BASE}${path}`, {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(body||{})}); return r.json(); }
...
function parseMultipart(req: any): Promise<{ file: { buffer: Buffer; filename?: string; mime?: string } | null; fields: any }> {
  return new Promise((resolve, reject)=>{
    ...
    bb.on("file", (_n, s, info)=>{ const chunks:Buffer[]=[]; s.on("data",(c:Buffer)=>chunks.push(c)); s.on("end",()=>{ out.file={ buffer:Buffer.concat(chunks), filename:info.filename, mime:info.mimeType };});});
...
const cors = (res:any)=>{ res.setHeader("Access-Control-Allow-Origin","*"); res.setHeader("Access-Control-Allow-Methods","POST, OPTIONS"); res.setHeader("Access-Control-Allow-Headers","Content-Type"); };

// ---------------------- compress <1MB ----------------------
async function compressUnder1MB(input: Buffer) {
  let sharp:any; try { const mod = await import("sharp"); sharp = mod.default||mod; } catch { return input; }
  try {
...
function toNumber(v:any): number|null {
  ...
  if (typeof v==="string"){ const m=v.replace(",",".").match(/-?\d+(\.\d+)?/); if(m) return Number(m[0]); }
  if (v && typeof v==="object"){ const n = toNumber(v.value ?? v.amount ?? v.quantity ?? v.qty ?? v.val ?? v.per_100g); if (isNum(n)) return n as number; }
  return null;
}
function deepFindNumberByKey(obj:any, pred:(k:string)=>boolean): number|null {
  if (!obj || typeof obj!=="object") return null;
  for (const k of Object.keys(obj)){ const val=obj[k], lk=k.toLowerCase(); if (pred(lk)){ const n=toNumber(val); if (isNum(n)) return n as number; } if (val && typeof val==="object"){ const r=deepFindNumberByKey(val,pred); if (isNum(r)) return r as number; } }
  return null;
}
function pickNumberByKey(obj:any, keys:(string|RegExp)[]){ for(const p of keys){ const t=typeof p==="string" ? (k:string)=>k===p.toLowerCase() : (k:string)=>(p as RegExp).test(k); const n=deepFindNumberByKey(obj,t); if (isNum(n)) return n; } return null; }
function pickNumberFromArrays(obj:any, regs:RegExp[], values=["value","amount","quantity","qty","val","per_100g","per100","per100g","kcal","g","mg"]){
  const stack=[obj]; while(stack.length){ const v=stack.pop(); if (!v||typeof v!=="object") continue;
    if (Array.isArray(v)){ for (const it of v){ if (it&&typeof it==="object"){ const label=String(it.name??it.label??it.tag??it.key??it.nutrient??it.id??"").toLowerCase();
        if (label && regs.some(rx=>rx.test(label))){ for (const vk of values){ if (vk in it){ const n=toNumber(it[vk]); if (isNum(n)) return n as number; } } }
      }
...
function getNutrient(obj:any, keys:(string|RegExp)[], arr?:RegExp[]){ const d=pickNumberByKey(obj,keys); if (isNum(d)) return d; const a=pickNumberFromArrays(obj, arr ?? keys.map(p=> typeof p==="string"? new RegExp(`\\b${p}\\b`) : p as RegExp)); return isNum(a)?a:null; }

function candidateScopes(n:any): any[]{ if(!n||typeof n!=="object") return []; const scopes:any[]=[];
  const push=(o:any)=>{ if(o&&typeof o==="object") scopes.push(o); };
  for (const k of Object.keys(n)) if (/(^|_|\s)per_?100g$|(^|_|\s)100g$|per_?100$/i.test(k)) push(n[k]);
  const stack=[n]; while(stack.length){ const v=stack.pop(); if(!v||typeof v!=="object") continue;
    if(Array.isArray(v)) v.forEach(x=>stack.push(x)); else for(const k of Object.keys(v)){ const c=v[k]; if (/(^|_|\s)per_?100g$|(^|_|\s)100g$|per_?100$/i.test(k)) push(c); if (c&&typeof c==="object") stack.push(c); } }
  scopes.push(n); return scopes;
}
function normalizePer100g(n:any){
  const scopes=candidateScopes(n);
  const run=(o:any)=>{
    const en={kcal:[/kcal\b/,/energy.*kcal/,/^calories?$/], protein:[/^protein/,/^prot\b/,/proteins?/], carb:[/^carb/,/carbo/,/carbohydrates?/], fat:[/^fat\b/,/lipid/], fiber:[/^fiber/,/fibre/], sugar:[/^sugar/], sodium:[/sodium/,/\bna\b/], potassium:[/potassium/,/\bk\b/], calcium:[/calcium/,/\bca\b/], iron:[/iron/,/\bfe\b/], vitd:[/vitamin[_\s-]?d/] };
    const es={kcal:[/calor[ií]as?/,/energ[ií]a/], protein:[/prote[ií]na(s)?/], carb:[/hidratos?/,/carbohidratos?/], fat:[/grasas?/], fiber:[/fibra/], sugar:[/az[uú]car(es)?/], sodium:[/sodio/], potassium:[/potasio/], calcium:[/calcio/], iron:[/hierro/], vitd:[/vitamina[_\s-]?d/]};
  ...
  for (const s of scopes){
    ...
    if (isNum(v.kcal)||anyMacro){
      ...
      return { calories:isNum(kcal)?kcal:null, protein_g:v.protein??null, carbs_g:v.carbs??null, fat_g:v.fat??null, fiber_g:v.fiber??null, sugars_g:v.sugar??null, sodium_mg:v.sodium??null, potassium_mg:v.potas??null, calcium_mg:v.calci??null, iron_mg:v.iron??null, vitamin_d_iu:v.vitd??null };
    }
  }
  return null;
}
const baseIsEmpty = (b:any)=> !b || [b.protein_g,b.carbs_g,b.fat_g,b.fiber_g,b.sugars_g,b.calories].every(v=>!isNum(v));
const zeros=()=>({calories:0,protein_g:0,carbs_g:0,fat_g:0,fiber_g:0,sugars_g:0,sodium_mg:0,potassium_mg:0,calcium_mg:0,iron_mg:0,vitamin_d_iu:0});
...
function per100FromIngredients(raw:any){ if(!raw) return null; const items:any[]=[]; const stack=[raw]; while(stack.length){ const v=stack.pop(); if(!v||typeof v!=="object") continue; if(Array.isArray(v)){ v.forEach(it=>{ if(it&&typeof it==="object"){ const p=normalizePer100g(it); if (p && !baseIsEmpty(p)) items.push(p);} if(it&&typeof it==="object") stack.push(it);}); } else for(const k of Object.keys(v)) stack.push(v[k]); } return items.length?avg(items):null; }

// ---------------------- food library ----------------------
type Nut = {calories:number, protein_g:number, carbs_g:number, fat_g:number, fiber_g?:number, sugars_g?:number, sodium_mg?:number, potassium_mg?:number, calcium_mg?:number, iron_mg?:number, vitamin_d_iu?:number};
...
const MATCHES: Array<[RegExp, keyof typeof F]> = [
  ...
  [/pan|bread|biscuit/i,"bread"], [/arroz|rice/i,"rice"],
  [/res|beef|carne molida/i,"beef"], [/pollo|chicken/i,"chicken"],
  [/champi|mushroom/i,"mushroom"], [/esp[aá]rrago|asparagus/i,"asparagus"],
...
function fillMissing(base:any, ref:any){ if(!base||!ref) return base; const out={...base}; for(const k of Object.keys(ref)){ if(!isNum(out[k])) out[k]=ref[k]; } return out; }
function sanitize(base:any, ref:any|null){
  ...
  if ((!isNum(out.calories) || out.calories>700) && ref){
...
export default async function handler(req:any,res:any){
  ...
  try {
    ...
    try { const payload=imageId?{imageId}:ingredients?{ingredients}:{};
